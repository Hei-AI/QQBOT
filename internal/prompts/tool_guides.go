...
func RootToolGuide() string {
	return strings.Join([]string{
		...
		"- QQ 群/私聊：决定发言时直接调用 send_message。message 必须非空；必须填写 targetType 和 targetId，取自要回复的 <qq_message> 标签。",
		...
		"- 聊天记录：需要核对某人的原话、说话人、时间或先前约定时调用 search_chat_history。它只检索最近最多 7 天保存过的 QQ 原始消息；优先带 query，或明确 targetType/targetId、userId，默认只返回少量结果。不要没事翻旧聊天。",
